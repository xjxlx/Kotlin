...
class AnimationJsonActivity : BaseBindingTitleActivity<ActivityAnimationJsonBinding>() {
    private var mWeakRef: WeakReference<LottieAnimationView>? = null
    ...
    override fun initData(savedInstanceState: Bundle?) {
        ...
        var animator: LottieAnimationView? = LottieAnimationView(this)
        mWeakRef = WeakReference(animator)
        /** 单个动画 **/
        mBinding.bt1AnimationSingle.setOnClickListener {
            ...
            var get = mWeakRef?.get()
            if (get == null) {
                get = LottieAnimationView(this)
                mWeakRef = WeakReference(animator)
            }
            mBinding.btn1.addView(get)
            get.setAnimation(R.raw.theme_background_dc)
            get.repeatCount = Int.MAX_VALUE
            get.playAnimation()
        } // 释放单个动画
        mBinding.bt1AnimationSingleSf.setOnClickListener {
            var get = mWeakRef?.get()
            if (get == null) {
                LogUtil.e("--------release----->")
            } else {
